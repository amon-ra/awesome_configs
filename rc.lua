...
function run_once(cmd, fake_cmd)
	...
	local pid = awful.spawn.pread("pgrep -u $USER -x " .. findme)
...
myleavemenu = {
	{"restart", "reboot", freedesktop.utils.lookup_icon({icon = 'system-reboot'})},
	{"shutdown", "poweroff", freedesktop.utils.lookup_icon({icon = 'system-shutdown'})}
}

table.insert(menu_items, {"awesome", myawesomemenu, beautiful.awesome_icon})
table.insert(menu_items, {"leave", myleavemenu, freedesktop.utils.lookup_icon({icon = 'boot'})})
...
vicious.register(tdwidget_text, vicious.widgets.date, '<span font="URW Gothic 10"> %b %d %H:%M </span>', 10)
...
vicious.register(volume_text, vicious.widgets.volume, '<span font="URW Gothic 10"> $1% </span>', 0.5, "Master")
...
globalkeys = awful.util.table.join(
	...
	awful.key({ }, "XF86PowerOff", function () awful.spawn.with_shell("poweroff") end),

	awful.key({ }, "XF86Launch1", function () awful.spawn.with_shell("reboot") end),
	...
	awful.key({modkey, "Shift"}, "#82", function () awful.tag.incnmaster(-1, nil, true) end),
	awful.key({modkey, "Control"}, "#86", function () awful.tag.incncol(1, nil, true) end),
	awful.key({modkey, "Control"}, "#82", function () awful.tag.incncol(-1, nil, true) end),
